  ...
  const loadOrders = async () => {
    try {
      ...
      if (res.success) {
        ...
        // Filter by status if tab is selected
        if (selectedTab) {
        ...
        {orders.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>
              {selectedTab ? `No ${getStatusLabelForHeader(selectedTab).toLowerCase()} orders yet` : "Start shopping to see your orders here"}
            </Text>
            {!selectedTab && (
              <TouchableOpacity
          ...
          orders.map((order) => (
            <OrderCard
              ...
              order={order}
              onPress={() => {
                router.push(`/consumer/orders/${order.order_id}`);
              }}
            />
          ))
        )}
      ...
      <View style={styles.orderCardBody}>
        <View style={styles.orderInfoRow}>
          ...
          <Text style={styles.orderInfoText} numberOfLines={1}>
            {order.farm_name || order.farmer_name || "Farmer"}
          </Text>
        </View>
